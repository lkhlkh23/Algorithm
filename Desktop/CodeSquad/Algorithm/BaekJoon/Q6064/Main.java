...
public class Main {
	...
	*/
	...
	public static void main(String[] args) {
		...
		int[] mArr = new int[caseNum];
		int[] nArr = new int[caseNum];
		int[] xArr = new int[caseNum];
		int[] yArr = new int[caseNum];
		
		for(int i = 0; i < caseNum; i++) {
			String str = getInput(sc);
			mArr[i] = getSplitInput(str, 0);
			nArr[i] = getSplitInput(str, 1);
			xArr[i] = getSplitInput(str, 2);
			yArr[i] = getSplitInput(str, 3);
		}
		
		for(int i = 0; i < caseNum; i++) {
			int result = -1;
			int cnt = (nArr[i] / gcd(mArr[i], nArr[i]));
			
			for(int j = 1; j <= cnt; j++) {
				int b = xArr[i] - yArr[i] + (mArr[i] * j) + nArr[i] - mArr[i];
				if(b%nArr[i] == 0) {
					result = xArr[i] + mArr[i] * (j - 1);
				}
			}
	...
	public static int getSplitInput(String str, int idx) {
